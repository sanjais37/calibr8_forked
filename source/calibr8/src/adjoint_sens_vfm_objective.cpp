...
namespace calibr8 {
  ...
  void Adjoint_VFM_Objective::gradient(
    ...
    double vp_mistach_scaled;
    double obj_dt_factor;
    double t_vp_factor;
    ...
    for (int step = nsteps; step > 0; --step) {
      dt = m_state[0]->disc->dt(step);
      obj_dt_factor = obj_scale_factor * dt / total_time;
      t_vp_factor = thickness * internal_power_scale_factor;
      load_at_step = m_load_data[step - 1];
      vp_mistach_scaled = (int_virtual_power_vec[step - 1] * thickness - load_at_step)
      * obj_scale_factor * dt / total_time;

      m_virtual_power->compute_at_step_grad(step, vp_mistach_scaled, internal_virtual_power,  grad_at_step);
