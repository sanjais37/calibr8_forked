...
void Adjoint_VFM_Objective::gradient(
  ...
  for (int step = 1; step <= nsteps; ++step) {
    dt = m_state[0]->disc->dt(step);
    m_virtual_power->compute_at_step(step);
  }

  for (int step = nsteps; step > 0; --step) {
  dt = m_state[0]->disc->dt(step);

  m_virtual_power->compute_at_step_grad(step, internal_virtual_power, grad_at_step);
  ...
  //  grad[i] += grad_at_step[i]* virtual_power_mismatch
   //     * obj_scale_factor * dt / total_time;
    grad[i] = 100; //grad_at_step[i]* virtual_power_mismatch
    //* obj_scale_factor * dt / total_time;
    }
  }
  m_J_old = J;
