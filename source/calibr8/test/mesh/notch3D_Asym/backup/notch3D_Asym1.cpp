...
int main()
{
    ...
    double thickness = 0.1; // Set the extrusion thickness
    std::vector<std::pair<int, int>> ov; // Output vector for new entities
    gmsh::model::geo::extrude({{2, 1}}, 0, 0, thickness, ov, {}, {}); // Extrude along the z-axis

    // Optionally, write the 3D mesh to a file
    write_closure_to_geo(f, "notch3D_Asym.geo");
    write_closure_to_dmg(f, "notch3D_Asym.dmg");
