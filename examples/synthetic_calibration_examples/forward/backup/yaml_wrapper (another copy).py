...
for i, data in enumerate(data_sets):
    ...
    # Ensure the structure exists and update only the relevant parts
    if 'residuals' in yaml_data and 'local residual' in yaml_data['residuals']:
        if 'materials' in yaml_data['residuals']['local residual'] and 'body' in yaml_data['residuals']['local residual']['materials']:
            yaml_data['residuals']['local residual']['materials']['body'].update(data)
        else:
            # If body or materials don't exist, initialize them
            yaml_data['residuals']['local residual']['materials'] = {'body': data}
    else:
        # Initialize the necessary structure
        yaml_data['residuals'] = {
            'local residual': {
                'materials': {'body': data}
            }
